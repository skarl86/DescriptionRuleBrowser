...
// nk : AST ALT Liverdisease
var testName = ['AST', 'ALT', 'LiverDisease'];
...
//       node = {id: ++lastNodeId, vals: testName[lastNodeId], reflexive: false};
//       // node = {id: 'AST', vals: defaultVals, reflexive: false};
//
...
var nodes = [
    ...
    lastNodeId = 8,
    links = [
      {source: nodes[0], target: nodes[1], left: false, right: true ,type: 0},
      {source: nodes[1], target: nodes[2], left: false, right: true ,type: 1},
      {source: nodes[1], target: nodes[3], left: false, right: true ,type: 1},
      {source: nodes[4], target: nodes[2], left: false, right: true ,type: 2},
      {source: nodes[5], target: nodes[2], left: false, right: true ,type: 2},
      {source: nodes[6], target: nodes[2], left: false, right: true ,type: 2},
      {source: nodes[7], target: nodes[2], left: false, right: true ,type: 2}
    ];
...
function setSelectedNode(node) {
  ...
  selectedNodeLabel.html(selected_node ? '<strong> '+selected_node.vals+ '</strong>' : 'No state selected');
...
function setValForSelectedNode2(varnum, value) {
//update buttons
  ...
  circle.selectAll('circle')
    //.style('fill', function(d) { return (d === selected_node) ? d3.rgb(colors(d.id)).brighter().toString() : colors(d.id); })
      .style('fill', function(d) {
        if(d.id === 0){
          return (d === selected_node) ? d3.rgb("#8B0000").toString() : d3.rgb("#8B0000").brighter().toString();
        }
        else {
          return (d === selected_node) ? d3.rgb(colors(1)).brighter().toString() : colors(1);
        }
      })
